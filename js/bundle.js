...
const TRANSIT_LABELS = [
  ...
  { lat: 46.4,  lng:   1.8, days: '17 to 25 days', route: 'route4' },
...
const LABEL_OFFSETS = {
  ...
  'ROTTERDAM': [12,0],
  'FRANKFURT': [12,-6], 'WARSAW': [12,-4], 'PARIS': [-52,8],
...
async function buildRoutes() {
  ...
  const osrmJobs = [
    ...
    fetchOSRMRoute(ROAD_SEGMENTS.route4_paris).then(c => c && addSegmentToRoute('route4', c, 'main')),
...
function addTransportIcons() {
  TRANSPORT_ICONS.forEach(({ lat, lng, icon, tip, route }) => {
    ...
    const html = `<div class="transport-svg-icon" style="background:${color}">${svg}</div>`;
    const el = L.divIcon({
